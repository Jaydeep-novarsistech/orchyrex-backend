...
const PartyWearShirt = ({ alphaUser, setAlphaUser }) => {
  ...
  <div className="bg-white">
      <div className="mx-auto max-w-2xl px-4 md:mb-[5rem] py-6 sm:px-6 mt-[8rem] sm:py-24 md:py-5 lg:max-w-7xl lg:px-8">
        <h1 className="text-4xl font-bold mb-6 text-center">Party-Wear Shirts</h1>
        ...
        {!loading && filteredProducts.length === 0 && (
          <p className="text-center text-gray-700">No Party-Wear shirts available at the moment.</p>
        )}
              ...
              {filteredProducts.map((item) => (
                <div key={item._id} className="group relative">
                  ...
                  <div className="mt-4 flex justify-between">
                    <div>
                      <h3 className="text-sm text-gray-900 font-bold">{item.desc}</h3>
                      <ReactStars
                        edit={false}
                        count={5}
                        value={item.rating}
                      ...
                      <p className="text-sm font-medium text-gray-900 mt-2">
                        <strike className="mr-5">&#8377; {item.price}</strike>
                        <span className="text-lg">&#8377; {item.discountedPrice}</span>
                      </p>
                    </div>
                    <div
